...
function getGuestCart() {
  try {
    return JSON.parse(
      localStorage.getItem("s2h_guest_cart") || "{}"
    );
  } catch {
    return {};
  }
...
export default function GuestCheckoutPage() {
  ...
  const [availabilityByProduct, setAvailabilityByProduct] =
    useState({});

  const [deliveryDateByProduct, setDeliveryDateByProduct] =
    useState({});
  ...
  // ---------------------------------------------------------
  // Create a key based on cart product IDs + quantities.
  //
  // IMPORTANT:
  // Including quantity here means availability is checked again
  // whenever the customer changes the quantity.
  // ---------------------------------------------------------
    ...
    const loadAvailability = async () => {
      ...
      await Promise.all(
        ids.map(async (id) => {
          ...
          try {
            // Pass the actual cart quantity AND the current
            // orderType — the backend now computes
            // earliest_delivery_date with the correct lead-time
            // rule for whichever fulfillment type is selected
            // (pickup skips min_lead_days entirely).
  ...
  const retryAvailability = async (id) => {
    ...
    try {
      // Get the CURRENT quantity from the cart, and pass the
      // current orderType so the backend applies the correct
      // lead-time rule.
      const quantity = Number(cart[id] || 0);
      ...
      if (data.earliest_delivery_date) {
        setDeliveryDateByProduct((prev) => ({
          ...prev,
          [id]: prev[id] || data.earliest_delivery_date,
        }));
      }
    } catch {
      setAvailabilityByProduct((prev) => ({
  ...
  // ---------------------------------------------------------
  // Block checkout when availability isn't resolved
  // ---------------------------------------------------------

  const availabilityBlockedItems =
    items.filter((i) => {
      const a =
        availabilityByProduct[i.id];
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      toast.success(
        `Order ${res.data.order.order_number} placed!`
      );
    } catch (err) {
      toast.error(
                ...
                {[
                  ...
                  <button
                    onClick={() => {
                        ...
                        // Pickup has no lead time, so don't retain
                        // a delivery date selected earlier.
                        if (t.key === "pickup") {
                          setDate("");
                        }
                      }}
